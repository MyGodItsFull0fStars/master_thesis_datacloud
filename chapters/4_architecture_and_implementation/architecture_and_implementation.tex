...
\label{sec:data-preprocessing-and-analysis-architecture}
  ...
  \label{sec:feature-engineering-data-preprocessing-architecture}
    ...
    It is a very important step in machine learning since the later prediction performance depends on the quality of data.
    The accuracy of an ML model heavily relies on a well composed and precise set of features order to make reliable predictions. Regardless of the data or ML architecture, if a feature set is not well-suited, it does have a direct impact on the ML model.
    \begin{quote}
    ...
    It can produce new features with the goal of enhancing the model accuracy and to simplify and speed up data transformation.
    ...
    \label{sec:feature-extraction-cleansing-preprocessing-architecture}
      ...
      The Alibaba cluster traces consist of a multitude of datasets, each with their respective focus. Because of the distributed nature of the data, first the common key features were required to be analysed in order to be able to later join the data based on this common key features to an actual training and test dataset.
    ...
    \end{quote}
    The one-hot encoding technique is both used for the categorical data of \emph{task type}, where each type of task is represented as an index in the task-type vector. Similarly, one-hot encoding is used to classify different number of instances (or Kubernetes Replicas) into clusters, that are then mapped to an index of the encoded feature vector.
    ...
    \label{sec:data-standardisation-data-preprocessing-architecture}
      ...
      In standardisation (also referred to as \emph{z-score normalisation}) is a process of scaling values of a dataset by first calculating its standard deviation and accounting for it. In case the standard deviation of features is different, then the range of those features will likewise differ. This has as a result that outliers in the characteristics are reduced.
    ...
    \label{sec:data-normalisation-data-preprocessing-architecture}
      ...
      Normalisation of data has no influence on the distribution of the features, but it does increase the effects of outliers due to the lower standard deviation after the modification. Because of this, it is necessary to deal with outliers before applying normalisation to a data set.
