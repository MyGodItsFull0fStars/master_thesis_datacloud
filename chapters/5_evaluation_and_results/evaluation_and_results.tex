...
\label{sec:data-analysis-evaluation}
  ...
  \label{sec:memory-data-analysis}
    ...
    % TODO Does this make sense from a statistical standpoint?
...
% what is common in the scenarios (or some of them)
  ...
  \label{sec:user-defined-hardware-utilisation}
      ...
      As seen in the \emph{OA} and \emph{UA} columns, the CPU over-allocation is at almost $80 \%$.
      \begin{table}
        \centering
    ...
    \label{sec:user-defined-memory-prediction-analysis}
      ...
      At the minimum amount, users predicted to require at least $1.95$ GB of memory, while the actual minimum was $20$ MB, which is almost $100$ times more than required for executing the task.
      In the quartiles, the trend to over-estimate the memory requirements continues whereas, in the first quartile, the factor of over-estimation is $2.59$ or $4.8$ GB.
      The amount of memory allocation is identical in both the second and third quartiles with $29.3$ GB.
      The factor of over-estimation for the second quartile is $6.26$ or $24.3$ GB and in the third quartile, the factor is $2.47$ or $17.5$ GB.
      Similar to the CPU utilisation, the memory utilisation predicted by the user is less than half as much as is actually required for the maximum value. In the dataset of actual memory utilisation, the maximum requires approximately $252$ GB, yet the user predicted a memory utilisation of $117$ GB.
      ...
      Similar to the CPU prediction, the over-allocation is predominant in the user-predicted memory allocation with $75.46\%$.
